...
export default function PlatformControlCenter() {
  ...
  const load = useCallback(
    async ({ quiet = false } = {}) => {
      ...
      try {
        ...
        const response = await fetch(`/api/v1/platform/control-center?${params}`, {
          cache: 'no-store',
        });
  ...
  const selectedSubscription = selected?.subscription_id
    ...
    : null;
  const latestJob = selected?.provisioning_job_id
    ? {
        ...
        error_code: selected.provisioning_error_code,
        error_message: selected.provisioning_error_message,
        started_at: selected.provisioning_started_at,
        finished_at: selected.provisioning_finished_at,
      }
    : null;
  ...
  const subscriptionColumns = [
    ...
    { title: 'Plan', dataIndex: 'plan_name' },
    {
      ...
      dataIndex: 'status',
      render: (value) => (
        <Tag color={subscriptionColor(value)}>{subscriptionLabel(value)}</Tag>
      ),
    },
  ...
  const channelColumns = [
    ...
    },
    {
      title: 'Provider',
      dataIndex: 'provider',
      render: (value) => (value === 'unconfigured' ? 'Not selected' : value),
    },
  ...
  const tabs = [
      ...
      children: (
        <Card styles={{ body: { padding: 0 } }}>
          <Table
            ...
            locale={{ emptyText: <Empty description="No companies have been added yet" /> }}
          />
        </Card>
      ),
    },
      ...
      children: (
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <Alert
            showIcon
            type="info"
            message="Plan records are ready"
            description="Plans and subscriptions already exist in the shared platform database. Creating, editing and assigning them is owned by Phase 12; this page makes the records visible now."
          />
          <Card title="Plans">
            <Table
              rowKey="id"
              ...
              pagination={false}
              locale={{ emptyText: <Empty description="No plans have been created yet" /> }}
            />
          </Card>
          <Card title="Subscriptions">
            <Table
              ...
              locale={{ emptyText: <Empty description="No subscriptions have been assigned yet" /> }}
            />
          </Card>
        </Space>
      ),
      ...
      children: (
        <Card>
          <Paragraph type="secondary">
            Every company setup request is recorded here. Failed work can be reviewed and safely
            retried.
          </Paragraph>
          <Table
            rowKey="id"
            ...
            locale={{ emptyText: <Empty description="No setup activity yet" /> }}
          />
        </Card>
      ),
    },
    {
      ...
      children: (
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <Card title="Platform administrators">
            <Table
              rowKey="id"
              columns={platformUserColumns}
              dataSource={data.platformUsers}
              pagination={false}
              locale={{ emptyText: <Empty description="No platform administrators found" /> }}
            />
          </Card>
          <Card title="Communication setup">
            <Table
              rowKey="id"
              columns={channelColumns}
              dataSource={data.channels}
              pagination={{ pageSize: 15, showSizeChanger: false }}
              locale={{ emptyText: <Empty description="No communication records yet" /> }}
            />
          </Card>
          <Card title="Platform feature controls">
            <Alert
              showIcon
              type="info"
              message="Management starts in Phase 12"
              description="Tenant-owned settings are already available inside each company. Platform-wide forced controls are visible here and become editable in Phase 12."
            ...
            <Table
              ...
              pagination={{ pageSize: 15, showSizeChanger: false }}
              locale={{ emptyText: <Empty description="No platform feature controls have been set" /> }}
            />
          </Card>
        </Space>
      ),
    ...
    <Layout style={{ minHeight: '100vh', background: '#f5f6f8' }}>
      ...
      <Content style={{ padding: 24, maxWidth: 1380, margin: '0 auto', width: '100%' }}>
        ...
        >
          <div>
            ...
            <Text type="secondary">Manage the companies that use Hippo Build.</Text>
          </div>
          <Space>
        ...
        >
          <Card>
            <Statistic title="All companies" value={data.summary.total} prefix={<TeamOutlined />} />
          </Card>
          <Card>
            <Statistic title="Ready" value={data.summary.active} prefix={<CheckCircleOutlined />} />
          </Card>
          <Card>
            <Statistic title="Setting up" value={data.summary.provisioning} prefix={<SettingOutlined />} />
          </Card>
          <Card>
            <Statistic title="Needs attention" value={data.summary.failed} prefix={<WarningOutlined />} />
          </Card>
        </div>
        ...
        <Card style={{ marginBottom: 18 }}>
          <Space direction="vertical" size={8}>
            <Text strong>
              <SafetyCertificateOutlined /> Automatic protection
            </Text>
            <Text>
              <CheckCircleOutlined /> Each company has its own private data area.
            </Text>
            <Text>
              <CheckCircleOutlined /> New company setup runs automatically without manual database
              work.
            </Text>
            <Text>
              <CheckCircleOutlined /> Database updates are applied and recorded for every company.
            </Text>
            <Text>
              <CheckCircleOutlined /> Email, SMS and WhatsApp passwords are encrypted and never shown
              in plain text.
            </Text>
          </Space>
        </Card>
      ...
      >
        <Alert
          type="info"
          showIcon
          message="Setup is automatic"
          description="Hippo Build creates the company workspace, prepares the system, creates the first administrator and communication settings, then marks the company ready."
        ...
        <Form form={form} layout="vertical" onFinish={createTenant} requiredMark={false}>
          ...
          <Form.Item
            label="Login name"
            name="slug"
            extra="A short lowercase name used on the login screen"
            rules={[
              { required: true },
              {
      ...
      <Drawer
        title={selected?.name || 'Company details'}
        ...
        {selected ? (
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            <Alert
              ...
              description={
                selected.status === 'active'
                  ? 'This company is ready to use Hippo Build.'
                  : selected.status === 'failed'
                    ? latestJob?.error_message || 'Setup needs attention.'
                    : `${setupStepLabel(latestJob?.current_step)}. This page refreshes automatically.`
              }
            />
            <Card title="Company" size="small">
              <Descriptions column={1} size="small">
                <Descriptions.Item label="Company name">{selected.name}</Descriptions.Item>
                <Descriptions.Item label="Login name">{selected.slug}</Descriptions.Item>
                <Descriptions.Item label="Current state">
                  <Tag color={tenantStatusColor(selected.status)}>
              ...
              <Text>
                {setupStepLabel(
                  latestJob?.current_step ||
                    (selected.status === 'active' ? 'active' : 'registered'),
                )}
              </Text>
              <Progress
            ...
            <Card title="Plan & subscription" size="small">
              {selectedSubscription ? (
                ...
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No plan is assigned yet" />
              )}
            </Card>
              ...
              items={[
                {
                  ...
                  children: (
                    <Descriptions column={1} size="small" bordered>
                      ...
                      <Descriptions.Item label="Storage folder">
                        <Text copyable>{selected.storage_prefix}</Text>
                      </Descriptions.Item>
                      <Descriptions.Item label="Isolation">
                        {selected.isolation_mode === 'dedicated_database'
                          ? 'Dedicated database'
                          : 'Private schema in shared database'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Database version">
                        {selected.migration_version || 'Pending'}
                      </Descriptions.Item>
                      <Descriptions.Item label="Data status">
                        {selected.data_location_status || '—'}
                      </Descriptions.Item>
                    </Descriptions>
                  ),
                },
